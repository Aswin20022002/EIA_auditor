...
export const SECTION_DEFS: SectionDef[] = [
  ...
  },
  {
    ...
    clause: "Ch. 6",
    headingPatterns: [headingRegex("environmental monitoring programm?e?"), headingRegex("post[- ]project monitoring")],
  ...
  },
  {
    ...
    clause: "Ch. 10",
    headingPatterns: [headingRegex("summary (?:and|&) conclusion"), headingRegex("summary of (?:the )?(?:eia|findings|project)")],
...
function isTableOfContentsLine(line: string): boolean {
  ...
  if (trimmed.length > 100) return false; // real headings are short
...
export function detectSections(fullText: string, pageOffsets?: number[]): DetectedSection[] {
  ...
  for (const def of SECTION_DEFS) {
    const hit = hits.find((h) => h.id === def.id);

    if (hit) {
      // Block runs until the next heading hit of any section (by char index).
      const nextHit = hits
        .filter((h) => h.charIndex > hit.charIndex)
      ...
      results.push({
        id: def.id,
        label: def.label,
        found: block.length >= MIN_CHARS_THIN,
        excerpt: block.slice(0, EXCERPT_CHARS),
        charCount: block.length,
        page: charIndexToPage(pageOffsets, hit.charIndex),
      });
      continue;
    }
